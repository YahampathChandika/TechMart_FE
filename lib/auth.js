// lib/auth.js - COMPLETE WITH PRIVILEGE SYSTEM
...
export const routeGuards = {
  ...
  requiresAdmin: (pathname) => {
    // All routes under /admin require authentication (not necessarily admin role)
...
// Permission helpers - UPDATED FOR PRIVILEGE SYSTEM
export const permissions = {
  ...
  // UPDATED: Check if user can perform product actions - handle both user and privileges param
  canAddProducts: (user, userPrivileges = null) => {
    // Admins can always add products
    if (user?.role === USER_ROLES.ADMIN) return true;

    // For regular users, check their specific privileges
    // If userPrivileges is not provided, get it from user object
    const privileges = userPrivileges || user?.privileges;
    return permissions.hasPrivilege(privileges, "can_add_products");
  },

  canUpdateProducts: (user, userPrivileges = null) => {
    // Admins can always update products
    if (user?.role === USER_ROLES.ADMIN) return true;

    // For regular users, check their specific privileges
    const privileges = userPrivileges || user?.privileges;
    return permissions.hasPrivilege(privileges, "can_update_products");
  },

  canDeleteProducts: (user, userPrivileges = null) => {
    // Admins can always delete products
    if (user?.role === USER_ROLES.ADMIN) return true;

    // For regular users, check their specific privileges
    const privileges = userPrivileges || user?.privileges;
    return permissions.hasPrivilege(privileges, "can_delete_products");
  },

  // NEW: Check if user can view admin data - ALL AUTHENTICATED USERS CAN VIEW
  canViewAdminData: (user) => {
    // Any authenticated admin or user can view data
  ...
  // Check if user can manage customers - ALL AUTHENTICATED USERS CAN VIEW
  canManageCustomers: (user) => {
    // All authenticated users can view customers
    return user?.role === USER_ROLES.ADMIN || user?.role === USER_ROLES.USER;
  },

  // NEW: Check if user can perform admin customer operations (admin only)
  canModifyCustomers: (user) => {
    return user?.role === USER_ROLES.ADMIN;
  },
};
...
export const sessionManager = {
  ...
  // Get current user type based on stored tokens
  getCurrentUserType: () => {
    const userToken = tokenManager.getUserToken();
    const customerToken = tokenManager.getCustomerToken();

    if (userToken) return "admin_user"; // Could be admin or regular user
...
export const authHelpers = {
  ...
  // Check if user is admin/user type
  isAdminUser: () => {
  ...
  isCustomer: () => {
    return !!tokenManager.getCustomerToken();
  },

  // Get appropriate auth header for API requests
  getAuthHeaders: () => {
    const userToken = tokenManager.getUserToken();
    const customerToken = tokenManager.getCustomerToken();
